...
def predict(image_path):
    model = tf.saved_model.load("model/yolov4-tiny-model", tags=[tag_constants.SERVING])
    orig_img = cv2.imread(image_path)
    orig_img = cv2.cvtColor(orig_img, cv2.COLOR_BGR2RGB)
    orig_width, orig_height,_ = orig_img.shape
    orig_img = cv2.resize(orig_img, (256, 256))
    img = np.array(orig_img).astype(np.float32)
    ...
    new_img = draw_bbox(orig_img, pred_bbox, show_label=True)
    new_img = Image.fromarray(new_img)
    new_img=new_img.resize((orig_height,orig_width))
    save_name = "saves/" + str(Path(image_path).stem) + "_output.jpg"
